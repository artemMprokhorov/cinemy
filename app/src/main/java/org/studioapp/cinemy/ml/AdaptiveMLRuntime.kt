...
class AdaptiveMLRuntime private constructor(private val context: Context) {
     ...
     * 
     * Processes multiple texts efficiently using the selected runtime.
     * This is useful for analyzing multiple reviews or comments at once.
     * 
    ...
    /**
     * Gets current runtime information
     * 
     ...
     */
    fun getCurrentRuntimeInfo(): String {
        val capabilities = hardwareDetection?.detectHardwareCapabilities()
        return buildString {
            appendLine("=== Current ML Runtime ===")
            appendLine("Runtime: ${currentRuntime}")
            appendLine("Performance Score: ${capabilities?.performanceScore ?: 0}/100")
            appendLine()
            appendLine("=== Hardware Capabilities ===")
            appendLine("GPU: ${capabilities?.hasGpu ?: false}")
            appendLine("NNAPI: ${capabilities?.hasNnapi ?: false}")
            appendLine("XNNPACK: ${capabilities?.hasXnnpack ?: false}")
            appendLine("LiteRT: ${capabilities?.hasLiteRT ?: false}")
            appendLine("Play Services: ${capabilities?.hasPlayServices ?: false}")
            appendLine()
            appendLine("=== Performance Expectations ===")
            when (currentRuntime) {
                HardwareDetection.MLRuntime.LITERT_GPU -> appendLine("Expected inference time: ~50ms")
                HardwareDetection.MLRuntime.LITERT_NPU -> appendLine("Expected inference time: ~80ms")
                HardwareDetection.MLRuntime.TENSORFLOW_LITE_GPU -> appendLine("Expected inference time: ~70ms")
                HardwareDetection.MLRuntime.TENSORFLOW_LITE_NNAPI -> appendLine("Expected inference time: ~100ms")
                HardwareDetection.MLRuntime.TENSORFLOW_LITE_CPU -> appendLine("Expected inference time: ~200ms")
                HardwareDetection.MLRuntime.KEYWORD_FALLBACK -> appendLine("Expected inference time: ~10ms (lower accuracy)")
                null -> appendLine("Runtime not determined")
            }
        }
    }
    ...
    fun getPerformanceRecommendations(): List<String> {
        return hardwareDetection?.getPerformanceRecommendations() ?: emptyList()
    }

    /**
     * Checks if runtime is optimal for sentiment analysis
     * 
    ...
    fun isOptimalForSentimentAnalysis(): Boolean {
        return hardwareDetection?.isOptimalForSentimentAnalysis() ?: false
    }
    ...
    fun getDeviceInfo(): String {
        return hardwareDetection?.getDeviceInfo() ?: "Hardware detection not available"
    }
